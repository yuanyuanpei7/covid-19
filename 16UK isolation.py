...
co=pd.read_csv('./16UK isolation.csv',encoding='gbk',header=0)
...
popt, pcov = optimize.curve_fit(fit_odeint, T[1:(len(y)-47)], y[1:(len(y)-47)], bounds=([0.01,0.01,0.01,0.01,0.15,0.06],[0.9,0.8,1,1,0.3,0.25]),maxfev=20000000)
...
plt.plot(x[(len(y) - 47):len(y)], fitted[(len(y) - 47):len(y)], 'b-', label='Prediction', linewidth=3)
plt.legend()
