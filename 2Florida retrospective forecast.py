...
co=pd.read_csv('./2Florida.csv',encoding='gbk',header=0)
...
plt.plot(x, y, 'r-', label='Infections', linewidth=1)
    #   plt.plot(mdates.drange(x.max()-dt.timedelta(days=45), x.max(), dt.timedelta(days=1)),fitted[len(y)-45:len(y)],'g-',label='simulation')
    #   plt.plot(mdates.drange(x.max(), x.max()+dt.timedelta(days=74), dt.timedelta(days=1)),fitted[len(y):],'r--',label='prognosis')
